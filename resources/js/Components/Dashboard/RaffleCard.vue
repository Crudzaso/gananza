<template>
  <div :class="['raffle-card shadow-lg rounded-lg p-4 flex flex-col items-center gap-4 mx-auto', theme.cardBackground]">
    <img src="../../../../public/assets/media/auth/Letra-Gananza.svg" alt="Premio"
          ...
          <DialogPanel :class="[theme.modalBackground, 'w-full max-w-3xl p-8 rounded-2xl shadow-2xl flex gap-8']">
            ...
            <div class="w-1/3 flex flex-col items-center justify-center bg-white p-4 rounded-lg shadow-lg">
              ...
              <button @click="proceedToPaymentModal"
                :class="[theme.buttonPrimary, 'px-4 py-2 rounded-lg']">Comprar</button>
            </div>
          </DialogPanel>
        </TransitionChild>
      </Dialog>
        ...
        <TransitionChild as="template" enter="ease-out duration-300" leave="ease-in duration-200">
          <DialogPanel :class="[theme.modalBackground, 'w-full max-w-lg p-8 rounded-2xl shadow-2xl']">
            ...
            <button @click="openVerificationModal"
              :class="[theme.buttonPrimary, 'px-4 py-2 rounded-lg mr-2']">Siguiente</button>
          ...
          <DialogPanel :class="[theme.modalBackground, 'w-full max-w-lg p-8 rounded-2xl shadow-2xl']">
            ...
            <form @submit.prevent="validatePayment">
              <div class="mb-4">
                ...
                <input v-model="referenceNumber" type="text" class="w-full p-2 border rounded-lg"
                  placeholder="Ingresa el número de comprobante" required />
              </div>
              <button type="submit" :class="[theme.buttonPrimary, 'px-4 py-2 rounded-lg']">Enviar</button>
            </form>
            <button @click="closeVerificationModal"
              :class="[theme.buttonDanger, 'px-4 py-2 rounded-lg mt-4']">Cerrar</button>
          </DialogPanel>
        </TransitionChild>
      </Dialog>
    </TransitionRoot>
