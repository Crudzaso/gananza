<template>
  <!-- Main Container -->
  <div
    :class="['raffle-card shadow-lg rounded-lg p-4 items-center gap-10 mx-auto grid grid-cols-2', theme.cardBackground]">
    <div>
      <!-- Countdown Timer -->
      <div class="countdown p-0.3 x-sm rounded-lg text-center"
      ...
      <img src="../../../../public/assets/media/auth/Logo-Gananza2.svg" alt="Prize"
        class="h-32 w-32 flex items-center justify-center rounded-lg mb-3" />
    </div>
    <!-- Raffle Title and Organizer -->
    <div>
      <h3 :class="theme.textSecondary">{{ raffle.name }}</h3>
      <p :class="theme.textSecondary">Organizador: <br> {{ raffle.organizer.name }}</p>
        ...
        <Dialog as="div" @close="closeWarningModal"
          ...
          <TransitionChild as="template" enter="ease-out duration-300" leave="ease-in duration-200">
            <DialogPanel :class="[theme.modalBackground, 'w-full max-w-md p-8 rounded-2xl shadow-2xl']">
              ...
              <p class="mb-4 p-14 text-xl text-center">¡Por favor, selecciona al menos un número antes de continuar con
                la compra!
              </p>
